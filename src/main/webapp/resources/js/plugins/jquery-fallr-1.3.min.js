/*Fallr v1.3.2 Copyright 2011-2012 amatyr4n, licensed under Envato licenses*/;
(function($) {
	var defaults = {
		buttons : {
			button1 : {
			...
			if (methods.isActive()) {
				...
				$f
						.animate(
								...
								function() {
									/**
									 * Harsh Panchal: Changes for latest jQuery
									 */
									//if ($.browser.msie) {
		...
		show : function(options, callback, self) {
			if (methods.isActive()) {
				...
				if (opts.useOverlay) {
					/**
					 * Harsh Panchal: Break IE Version < 9 support as newer version of jquery
					 * has changed some API
					 * 
				...
				$f.find(':input').bind('keydown', function(e) {
					...
					if (e.keyCode === 13) {/* $('.fallr-button').eq(0).trigger('click'); */
						return false;
					}
				});
			}
