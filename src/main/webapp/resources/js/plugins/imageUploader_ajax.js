...
// send data
function ajaxUpload(form,status){
	var detectWebKit = isWebKit();
    /*----- status = 1 for upload attachment ---------------*/
    if(status == 1){
        var get_url = 'attachment_upload.php';
    }/*----- status = 0 for upload images ---------------*/
    else{
        var get_url = 'image_upload.php';
    }
	var div_id = 'upload_area';// div id where to show uploaded image
	var show_loading = '<img src="images/loadingCustom.gif" />';// loading image
	var html_error = '';// error image
