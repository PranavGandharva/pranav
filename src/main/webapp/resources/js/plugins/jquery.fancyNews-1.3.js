...
/* Added '<div class='clear'></div><a href='"+links[i]+"' target='_blank' class='readMore'>read more</a>' in the preview[i] code - ALM */
		...
		var $element, conHeight, scrollPaneApi, slideInt, titles = [], dates = [], summaries = [], descriptions = [], links = [], previewCons = [], topConIndex = 0, busy = true, isSliding = false, slideDirection = "next";
			...
			function _loadFromDom(domList){
				...
				for(var i=0; i < domList.length; ++i){
					...
					previews[i] = "<div class='fn-newsPreview' style='width:"+options.width+"px; height:"+conHeight+"px; background:"+options.backgroundColor+";'>"+imageDom+"<span class='fn-newsPreviewText' ><h4>"+dates[i]+"</h4><h3>"+titles[i]+"</h3><p>"+summaries[i]+"</p><a href='#' target='_blank' class='readMore'>read more</a></span></div>";
				};
			...
			function _loadFromXml(){
				...
				$.jGFeed(options.feed,function(feeds){
					...
					for(var i=0; i<feeds.entries.length; i++){
					  ...
					  previews[i] = "<div class='fn-newsPreview' style='width:"+options.width+"px; height:"+conHeight+"px; background:"+options.backgroundColor+";'><span class='fn-newsPreviewText' ><h4>"+dates[i]+"</h4><h3>"+titles[i]+"</h3><p>"+summaries[i]+"</p></span><div class='clear'></div><a href='"+links[i]+"' target='_blank' class='readMore'>read more</a></div>";
					}
			...
			function _createPreviews(previews){
				...
				for(var i=0; i<options.previewsPerPage;++i){
					$(previewCons[i]).hide().delay((200+(i*200))).show();
				};
